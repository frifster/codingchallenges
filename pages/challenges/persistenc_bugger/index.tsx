...
function persistence(num: number): number {
  ...
  const splitNum = numToString.split("");

  const productOfNums = splitNum.reduce((product, currentNum) => {
    return Number(currentNum) * product;
  }, 1);
  ...
  return persistence(productOfNums) + 1;
}
